...
const simpleGraphQlJoinQueryString = `query humansCarsJoin { Humans { id name height Cars { ownerId ownerId @childJoinColumn id @parentJoinColumn model year } Laptops { ownerId ownerId @childJoinColumn id @parentJoinColumn model screenSize } Houses { ownerId @childJoinColumn id @parentJoinColumn number street } } }`;
